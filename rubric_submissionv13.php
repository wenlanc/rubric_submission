...
<?php if($flag){ ?>
        ...
        <tr style="background:<?= ($assignment_list[$i]['id'] == $assignment_id)?"#337ab7":"#58b6ed" ?>">
            ...
            <td>
				...
				<form action="?" method="post">
					...
					<input value="<?=$token?>" name="access_token" type="hidden">
					<input value="<?=$assignment_list[$i]['id']?>" name="assignment_id" type="hidden">
					<input class="btn btn-secondary btn-sm"  role="button"" value="Select" type="submit">
				<form/>
                <?php    
                    } 
                ?>
    ...
    ?>
		...
		<form action="?" method="post">
			<input value="<?=$course_id?>" name="course_id" type="hidden">
			<input value="<?=$token?>" name="access_token" type="hidden">
			<input value="<?=$assignment_id?>" name="assignment_id" type="hidden">
			<input value="" name="criteria_ids" id="criteria_ids" type="hidden">
			<input class="btn btn-primary btn-lg btn-block" id="printBtn" role="button"" value="Print to Excel" type="submit">
		<form/>
    <?php
    }
    ?>
...
<script>
	...
	$(document).ready(function() {
		$('input.criteria_check').change(function() {
			...
			$('input.criteria_check').each(function() {
			  if ($(this).is(':checked')) {
				...
				$("#criteria_ids").val(JSON.stringify(criteria_ids)); 
